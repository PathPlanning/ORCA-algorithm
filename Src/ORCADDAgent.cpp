...
void ORCADDAgent::ComputeNewVelocity()
{
    ...
    for (int i = 0; i < NeighboursObst.size(); i++)
    {
        ...
        {
            if ((lRelativePosition * invTimeBoundaryObst - ORCALines[j].liesOn).Det(ORCALines[j].dir) - invTimeBoundaryObst * effectiveRadius >= -CN_EPS &&
                (rRelativePosition * invTimeBoundaryObst - ORCALines[j].liesOn).Det(ORCALines[j].dir) - invTimeBoundaryObst * effectiveRadius>= -CN_EPS)
            {
                alreadyCovered = true;
                break;
            }
        ...
        if (s < 0.0f && lineSqDist <= sqEfRadius)
        {
            ...
            lLegDirection = Point(lRelativePosition.X() * leg1 - lRelativePosition.Y() * effectiveRadius, lRelativePosition.X() * effectiveRadius + lRelativePosition.Y() * leg1) / lSqDist;
            rLegDirection = Point(lRelativePosition.X() * leg1 + lRelativePosition.Y() * effectiveRadius, -lRelativePosition.X() * effectiveRadius + lRelativePosition.Y() * leg1) / lSqDist;
        }
        else if (s > 1.0f && lineSqDist <= sqEfRadius)
        {
            ...
            lLegDirection = Point(rRelativePosition.X() * leg2 - rRelativePosition.Y() * effectiveRadius, rRelativePosition.X() * effectiveRadius + rRelativePosition.Y() * leg2) / rSqDist;
            rLegDirection = Point(rRelativePosition.X() * leg2 + rRelativePosition.Y() * effectiveRadius, -rRelativePosition.X() * effectiveRadius + rRelativePosition.Y() * leg2) / rSqDist;
        }
        else
        {
            if (left->IsConvex())
        ...
        const float t = (right == left ? 0.5f : ((effectiveVelocity - leftCutoff).ScalarProduct(cutoffVec)) / cutoffVec.SquaredEuclideanNorm());
        ...
        float cutoffSqDist = ((t < 0.0f || t > 1.0f || right == left) ? std::numeric_limits<float>::infinity() : (effectiveVelocity - (leftCutoff + cutoffVec * t)).SquaredEuclideanNorm());
        float lLegSqDist = ((tLeft < 0.0f) ? std::numeric_limits<float>::infinity() : (effectiveVelocity - (leftCutoff + lLegDirection * tLeft)).SquaredEuclideanNorm());
        float rLegSqDist = ((tRight < 0.0f) ? std::numeric_limits<float>::infinity() : (effectiveVelocity - (rightCutoff + rLegDirection * tRight)).SquaredEuclideanNorm());
...
void ORCADDAgent::ComputeWheelsSpeed()
{
    ...
    float abs = newV.EuclideanNorm();
    float sin = 0.0, cos = 1.0;
    if(abs != 0.0)
    {
        float invAbs = 1 / newV.EuclideanNorm();
        sin = newV.Y() * invAbs;
        cos = newV.X() * invAbs;
    }
    float invL = 1 / wheelTrack;
    float A1 = cos * 0.5f + D * sin * invL;
    float A2 = sin * 0.5f - D * cos * invL;
    float B1 = cos * 0.5f - D * sin * invL;
    float B2 = sin * 0.5f + D * cos * invL;

    float  divB = B1 * (1/B2);

    float invden = 1 / (A1 - A2 * divB);
    this->leftV = (newV.X() - newV.Y() * divB) * invden;
    this->rightV = (newV.Y() - A2 * leftV) * (1/B2);
    float avg = (leftV + rightV) * 0.5f;
    currV = Point(avg * cos, avg * sin);
}
