...
class PushAndRotate : public MAPFSearchInterface
{
    public:
        ...
        void clear() override;
    private:
        bool solve(const SubMap &map, const MAPFConfig &config, MAPFActorSet &AgentSet, std::chrono::steady_clock::time_point start);
        ...
        bool multipush(const SubMap &map, MAPFActorSet &agentSet, Node first, Node second, Node& to, std::list<Node>& path);
