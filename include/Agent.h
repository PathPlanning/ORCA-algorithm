...
class Agent
{
    public:
        ...
        virtual void computeNewControl() =0;
        virtual void applyNewControl() = 0;
        virtual bool prepareBeforeStep() = 0;

        virtual void updatePosition(const Point &pos);
        virtual void addNeighbour(Agent &neighbour, float dist_sq);
        ...
        Point getNextGoalPoint() const;


        std::pair<unsigned int, unsigned int> getCollisionCount() const;
        ...
        template <class Planner> void setGlobalPlanner(const Planner &pl)
        {
