...
//                std::cout << "\nSuccess\tRuntime\tMakespan\tFlowtime\tCollisions\tCollisionsObst\tMeanMAPFTime\tInits\tUpdates\tUnites\tECBS\tPnR\n";
				if (first_flag) {
					for (auto &key: keys) {
						std::cout << std::right << std::setfill(' ') << std::setw(20) << key << ' ';
						pre_log << std::right << std::setfill(' ') << std::setw(20) << key << ' ';
					}
					std::cout << std::endl;
					pre_log << std::endl;
					first_flag = false;
				...
				for (auto &key : keys) {
					std::cout << std::right << std::setfill(' ') << std::setw(20) << full_summary[key]  << ' ';
					pre_log << std::right << std::setfill(' ') << std::setw(20) << full_summary[key]  << ' ';
				}
				std::cout << std::endl;
				pre_log << std::endl;
