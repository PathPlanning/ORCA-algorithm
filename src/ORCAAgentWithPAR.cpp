...
void ORCAAgentWithPAR::computeNewControl()
{
    if(PARExec)
    {
     // Collision count
...
void ORCAAgentWithPAR::applyNewControl()
{
    PARVis = false;
    notPARVis = false;
    PARUnion = false;
...
bool ORCAAgentWithPAR::prepareBeforeStep()
{
...
void ORCAAgentWithPAR::addNeighbour(Agent &neighbour, float dist_sq)
{
    ...
    if(i < param.agentsMaxNum)
    {
        ...
        if(tmpAgentPAR->inPARMode)
        {
            ...
            if(inPARMode)
            {
                ...
                {
                    if(dist_sq < (param.radius + tmpAgentPAR->param.radius + 0.25) * (param.radius + tmpAgentPAR->param.radius + 0.25))
                    {
                        PARUnion = true;
                    }
                }
