...
void ORCAAgentWithPARAndECBS::computeNewControl()
{
    if(MAPFExec)
    {
        // Collision count
...
void ORCAAgentWithPARAndECBS::applyNewControl()
{
    MAPFVis = false;
    notMAPFVis = false;
    MAPFUnion = false;
...
bool ORCAAgentWithPARAndECBS::prepareBeforeStep()
{
    Point next;
    if(inMAPFMode)
    {
...
void ORCAAgentWithPARAndECBS::addNeighbour(Agent &neighbour, float dist_sq)
{
    ...
    if(i < param.agentsMaxNum)
    {
        ...
        if(tmpAgentMAPF->inMAPFMode)
        {
            ...
            if(inMAPFMode)
            {
                ...
                {
                    if(dist_sq < (param.radius + tmpAgentMAPF->param.radius + 0.25) * (param.radius + tmpAgentMAPF->param.radius + 0.25))
                    {
                        MAPFUnion = true;
                    }
                }
